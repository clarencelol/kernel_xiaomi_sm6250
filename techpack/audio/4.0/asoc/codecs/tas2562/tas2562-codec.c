				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret < 0)
		dev_dbg(p_tas2562->dev, "can't get ivsensor state %s, L=%d\n",
			__func__, __LINE__);
	...
	dev_dbg(p_tas2562->dev, "value: 0x%x, tas2562iv_enable %d\n",
			...
			enum channel chn, int state)
{
	...
	dev_dbg(p_tas2562->dev, "set power state: %d\n", state);
	...
	default:
		dev_dbg(p_tas2562->dev, "wrong power state setting %d\n",
				state);
	}
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		dev_dbg(p_tas2562->dev, "SND_SOC_DAPM_POST_PMU\n");
		break;
	case SND_SOC_DAPM_PRE_PMD:
		dev_dbg(p_tas2562->dev, "SND_SOC_DAPM_PRE_PMD\n");
		break;
					...
					struct snd_ctl_elem_value *p_u_control)
{
	...
	dev_dbg(p_tas2562->dev, "%s, p_u_control = %ld\n",
					...
					struct snd_ctl_elem_value *p_u_control)
{
	...
	dev_dbg(p_tas2562->dev, "%s, spk_l_control = %d\n",
					...
					struct snd_ctl_elem_value *p_u_control)
{
	...
	dev_dbg(p_tas2562->dev, "%s, spk_r_control = %d\n",
...
static int tas2562_iv_slot_config(struct tas2562_priv *p_tas2562)
{
	...
	if (p_tas2562->mn_channels == 2) {
		...
		dev_dbg(p_tas2562->dev, "%s, wrong params, %d\n",
...
static int tas2562_set_slot(struct tas2562_priv *p_tas2562, int slot_width)
{
	...
	default:
		dev_dbg(p_tas2562->dev, "slot width not supported");
		ret = -EINVAL;
	}
...
static int tas2562_set_bitwidth(struct tas2562_priv *p_tas2562, int bitwidth)
{
	...
	default:
		dev_dbg(p_tas2562->dev, "Not supported params format\n");
	}
			...
			int samplerate)
{
	...
	case 17640:
		...
		p_tas2562->update_bits(p_tas2562, channel_both,
			...
			break;
	default:
			dev_dbg(p_tas2562->dev, "%s, unsupported sample rate, %d\n",
			__func__, samplerate);
	}
		...
		struct snd_soc_dai *dai)
{
	...
	n_result = tas2562_set_bitwidth(p_tas2562,
			params_format(params));
	if (n_result < 0) {
		dev_dbg(p_tas2562->dev, "set bitwidth failed, %d\n", n_result);
		goto ret;
	}
...
static int tas2562_set_fmt(struct tas2562_priv *p_tas2562, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBS_CFS:
		...
		break;
	default:
		dev_dbg(p_tas2562->dev, "ASI format master is not found\n");
		ret = -EINVAL;
	}

	switch (fmt & SND_SOC_DAIFMT_INV_MASK) {
	case SND_SOC_DAIFMT_NB_NF:
		dev_dbg(p_tas2562->dev, "INV format: NBNF\n");
		asi_cfg_1 |= TAS2562_TDMCONFIGURATIONREG1_RXEDGE_RISING;
		break;
	case SND_SOC_DAIFMT_IB_NF:
		dev_dbg(p_tas2562->dev, "INV format: IBNF\n");
		asi_cfg_1 |= TAS2562_TDMCONFIGURATIONREG1_RXEDGE_FALLING;
		break;
	default:
		dev_dbg(p_tas2562->dev, "ASI format Inverse is not found\n");
		ret = -EINVAL;
	}
	...
	case (SND_SOC_DAIFMT_LEFT_J):
		...
		break;
	default:
	dev_dbg(p_tas2562->dev, "DAI Format is not found, fmt=0x%x\n", fmt);
	ret = -EINVAL;
		break;
	}
